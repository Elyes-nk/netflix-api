...
module.exports = {
    Query: {
        getLists: () => {
            return List.find().catch(err => console.log(err)).populate("genre").populate("content");
        },
        getList: (parent, args) => {
            return List.findById(args.id).catch(err => console.log(err)).populate("genre").populate("content");
        }
    },
    Mutation: {
